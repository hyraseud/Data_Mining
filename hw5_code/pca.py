...
class PCA(object):
    ...
    def fit(self, X) -> None:
        ...
        # >> YOUR CODE HERE
        cov = self.cov(X.T)
        eigen_values, eigen_vectors = self.eig(cov.T)
        self.components = eigen_vectors[:,:self.n_components]
        #print(self.components.shape)
        total_val = sum(eigen_values)
        self.explained_variance_ratio = np.array([(i/total_val) for i in sorted(eigen_values, reverse=True)])
        self.explained_variance_ratio = self.explained_variance_ratio[:self.n_components]
        #print(self.explained_variance_ratio.shape)
        #print("done fit")
        # << END OF YOUR CODE
    ...
    def inverse_transform(self, X: np.ndarray, X_pca: np.ndarray) -> np.ndarray:
        ...
        #print(X_pca.shape)
        #print(vec.T.shape)
        return  np.dot(np.array(X_pca), np.array(vec.T)) + mean
        # << END OF YOUR CODE
