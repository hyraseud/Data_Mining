...
def pre_process(file_path: str) -> dict:
    ...
    for stock in stocks:
        # >>> YOUR CODE HERE
        stock_dict[stock] = df[df.Stock == stock].sort_values(by='Date', ascending=True)
        # <<< END YOUR CODE
   # print(stock_dict)
    return stock_dict
...
def split_stock(stock_info: pd.DataFrame) -> tuple:
    ...
    """
    y = stock_info.pop('Date')
    x = stock_info
    
    #debug

    # >>> YOUR CODE HERE
    x_train_stock, x_val_stock, y_train_stock, y_val_stock = train_test_split(x, y, test_size=0.3, shuffle=False)

    # <<< END YOUR CODE
    return (x_train_stock, y_train_stock, x_val_stock, y_val_stock)


def slice_num(s: pd.DataFrame(), n: int):
    stock = np.array(s)
    mask = list(range(n-1,len(stock),n))
    return np.delete(stock, mask), stock[mask]
...
def get_train_valid(stock_dict: dict) -> tuple:
    ...
    for stock in stock_dict:
        s = stock_dict.get(stock)['Close']
        #d = stock_dict.get(stock)['Date']
        test, train = slice_num(s, 5)
        #date_test, date_train = slice_num(d, 5)
        
        if stock == 'AAPL':
            appl_test = np.array(test, dtype=('float64'))
            appl_train = np.array(train, dtype=('float64'))
            
            
        if stock == 'MSFT':
            m_test = np.array(test, dtype=('float64'))
            m_train = np.array(train, dtype=('float64'))
            
            
        if stock == 'TSLA':
            t_test = np.array(test, dtype=('float64'))
            t_train = train
          
            
        if stock == 'FB':
            f_test = np.array(test, dtype=('float64'))
            f_train = np.array(train, dtype=('float64'))

    x = np.hstack((appl_test, m_test, t_test, f_test))
    y = np.hstack((appl_train, m_train, t_train, f_train))
...
def train(data: tuple, max_epochs: int = 200, seed=12345) -> tuple:
    ...
    optimizer = optim.SGD(net.parameters(), lr=0.0001) # DEFINE YOUR OPTIMIZER
    ...
    for epoch in iterator: # DO NOT CHANGE THIS LINE
        ...
        #print(x_t)
        #print(outputs)
        train_loss = criterion(outputs, x_t)
        val_loss = criterion(outputs, y_t)
        optimizer.step()
        optimizer.zero_grad()
        # <<< END YOUR CODE
...
def plot_predictions(model: nn.Module, stock_dict: dict) -> None:
    ...
    for k, stock in enumerate(list(stock_dict.keys())):
        ...
        pred = model(torch.Tensor(x_val.values).to(device)).detach().cpu().numpy(dtype='float64')
